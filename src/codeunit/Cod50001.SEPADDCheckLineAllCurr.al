codeunit 50001 "SEPA DD-Check Line All Curr."
{
    ...
    var
        ...
        IsHandled: Boolean;
        DDC: Record "Direct Debit Collection";
    begin
        IsHandled := false;
        ...
        GLSetup.Get();
        DDC.Reset();
        DDC.SetFilter(Status, '<>%1&<>%2', DirectDebitCollection.Status::Posted, DirectDebitCollection.Status::Closed);
        if DDC.FindFirst() then
            repeat
                DirectDebitCollectionEntry.SetRange("Direct Debit Collection No.", DDC."No.");
